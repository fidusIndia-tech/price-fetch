...
def init_schema():
    ...
    try:
        cur.execute("""
            ...
            INSERT INTO users(username,password) SELECT 'admin', 'CHANGEME_ON_FIRST_LOGIN'
    ...
    with col_a:
        ...
        if st.button("✚ Create User", use_container_width=True):
            if nu and np_:
                c = get_conn(); cur = c.cursor()
                try:
                    cur.execute("INSERT INTO users(username,password) VALUES(%s,%s)", (nu, hash_password(np_)))
                ...
                try:
                    ...
                    if new_admin_pass:
                        cur.execute(
                            ...
                            (new_admin_user, hash_password(new_admin_pass), username)
                        )
                    else:
                        cur.execute(
